#!/usr/bin/env python3
"""Train and evaluate an ensemble of Conditional Variational Autoencoders (CVAEs) for anomaly detection.

This script trains an ensemble of three CVAE models on specified data splits (50/50, 60/40, 90/10) using
mixed precision training on a GPU. It processes video frame data from the UNCC CHAD dataset, employing
focal loss to address class imbalance, and generates loss curves as PNG files for visualization. The
script saves model checkpoints and logs training/validation metrics, designed for a dissertation project
evaluating anomaly detection performance across imbalanced datasets.
...
Attributes:
    ...
    SPLIT (str): The data split to train on (e.g., '5050', '6040', '9010'), parsed from command-line.
    SPLIT_DIR (str): Directory mapping for the split (e.g., '5050' -> '5050', '6040' -> '60_40').

Args:
    --split (str): Required argument specifying the data split to train on. Choices are '5050', '6040',
                   or '9010'. Defaults to '5050' if not specified.

Returns:
    None: The script saves model checkpoints to CHECKPOINT_DIR, loss data to CSV files, generates
          loss curves as PNG files (e.g., /home/sanderwald/Projects/dissertationProject/loss_curve_5050.png),
          and logs the process to train_ensemble_cvae_{SPLIT}.log.

Raises:
    ValueError: If no valid image paths are found after filtering the train_split_subset.txt file.
    RuntimeError: If the batch count exceeds the expected total during training.
    Exception: For other unforeseen errors during training, validation, or file operations.

Example:
    python evaluate_ensemble_cvae.py --split 5050

Notes:
    - Requires a GPU-enabled environment (e.g., tf_gpu Conda environment with TensorFlow 2.10 and Python 3.10).
    - Assumes preprocessed image data in MAIN_IMAGE_DIR and split files in TRAIN_FILE path.
    - Uses gdown for potential data download (not implemented here but referenced in project context).
    - GPU memory is optimized with set_memory_growth(True) and a 1500MiB limit per device.
    - Checkpoints are saved in CHECKPOINT_DIR/epoch_{epoch}/cvae_{i} format.
...
parser.add_argument('--split', type=str, default='5050', choices=['5050', '6040', '9010'], help="Specify the split: 5050, 6040, or 9010")
args = parser.parse_args()

SPLIT = args.split
SPLIT_DIR = {'5050': '5050', '6040': '60_40', '9010': '90_10'}[SPLIT]
...
CONFIG = {
    ...
    "CHECKPOINT_DIR": f"outputs_{SPLIT}_ensemble_cvae_20250621",
    "IMG_SIZE": (360, 203),
    "BATCH_SIZE": 1,
    "LATENT_DIM": 4,
    ...
    "NUM_CVAES": 3,
    "EPOCHS": 3,
    "NUM_SAMPLES": {"5050": 10000, "6040": 24720, "9010": 10000}[SPLIT]  # Reduced 9010 to match 5050 workaround
}
...
def plot_losses(train_losses, val_losses, split):
    epochs = range(1, CONFIG["EPOCHS"] + 1)
    plt.figure(figsize=(10, 6))
    plt.plot(epochs, [tl.numpy() for tl in train_losses], label='Training Loss')
    plt.plot(epochs, [vl.numpy() for vl in val_losses], label='Validation Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Loss Curves for {split} Split')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"/home/sanderwald/Projects/dissertationProject/loss_curve_{split}.png")
    plt.close()

def main():
    logging.info(f"Starting training for {SPLIT} split (ensemble)")
    try:
        ...
        for i in range(CONFIG["NUM_CVAES"]):
            ...
            models.append(model)
            logging.info(f"Initialized model {i} for {SPLIT}")
            tf.keras.backend.clear_session()
        ...
        logging.info(f"Optimizer built with {len(all_trainable_vars)} trainable variables for {SPLIT}")
        ...
        for epoch in range(1, CONFIG["EPOCHS"] + 1):
            logging.info(f"Starting epoch {epoch}/{CONFIG['EPOCHS']} for {SPLIT}")
            total_loss = 0.0
            total_recon_loss = 0.0
            total_kl_loss = 0.0
            total_focal_loss = 0.0
            ...
            for x_batch, y_batch in train_dataset:
                ...
                if global_batch_count > total_expected_batches:
                    logging.error(f"Exceeded expected total batches ({total_expected_batches}) for {SPLIT}. Stopping training.")
                    raise RuntimeError("Batch count exceeded expected total")
                logging.info(f"Processing batch {batch_count}/{train_batches} (Global: {global_batch_count}/{total_expected_batches}) for {SPLIT}")
                batch_loss = 0.0
                batch_recon_loss = 0.0
                batch_kl_loss = 0.0
                batch_focal_loss = 0.0
            ...
            logging.info(f"Running validation for epoch {epoch} for {SPLIT}")
            ...
            for i, model in enumerate(models):
                ...
                tf.keras.backend.clear_session()
        plot_losses(train_losses, val_losses, SPLIT)
